...
resource "google_compute_network" "vpc" {
  name                    = "${var.gcp_app_name}-${var.gcp_environment}-vpc"
...
resource "google_compute_subnetwork" "private_subnet_1" {
  ...
  purpose       = "PRIVATE"
  name          = "${var.gcp_app_name}-${var.gcp_environment}-private-subnet-1"
...
resource "google_compute_address" "nat_ip" {
  name    = "${var.gcp_app_name}-${var.gcp_environment}-nap-ip"
...
resource "google_compute_router" "nat-router" {
  name    = "${var.gcp_app_name}-${var.gcp_environment}-nat-router"
  network = google_compute_network.vpc.name
}
...
resource "google_compute_firewall" "allow-internal" {
  name    = "${var.gcp_app_name}-${var.gcp_environment}-fw-allow-internal"
  network = "${google_compute_network.vpc.name}"
  allow {
    protocol = "icmp"
  }
